...
import { getAmbientTemperature, isAuthenticated } from '../services/ecobeeApi';
...
export function ProofingScreen() {
  ...
  const [loadingEcobee, setLoadingEcobee] = useState(false);
  ...
  const handleEcobeePull = useCallback(async () => {
    setLoadingEcobee(true);
    try {
      const authenticated = await isAuthenticated();
      if (!authenticated) {
        Alert.alert(
          'Ecobee Not Connected',
          'Go to Settings to connect your Ecobee thermostat.',
          [{ text: 'OK' }]
        );
        return;
      }
      ...
      Alert.alert('Ecobee Error', error.message);
    } finally {
      setLoadingEcobee(false);
    }
  }, []);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.calcCard}>
        ...
        {/* Ecobee button */}
        <TouchableOpacity
          style={styles.ecobeeButton}
          onPress={handleEcobeePull}
          disabled={loadingEcobee}
        >
          {loadingEcobee ? (
            <ActivityIndicator color={colors.golden} size="small" />
          ) : (
            <Text style={styles.ecobeeText}>🌡 Pull from Ecobee Thermostat</Text>
          )}
        </TouchableOpacity>
      </View>
